...
function choix_image_vendeur(){
    ...
    $(document).on('change', '.btn_choix_img_Ajoutvendeur :file', function() {
        let maVariable = $(this),
            nvx_texte = maVariable.val().replace(/\\/g, '/').replace(/.*\//, '');
            maVariable.trigger('fileselect', [nvx_texte]);
    });

    $('.btn_choix_img_Ajoutvendeur :file').on('fileselect', function(event, nvx_texte) {
        
        let nom_img = $(this).parents(".img_Ajoutvendeur_general").find(':text'),
    ...
    function readURL(mon_img) {
        if (mon_img.files && mon_img.files[0]) {
            ...
            read.onload = function (e) {
                $("#img_Ajoutvendeur").attr('src', e.target.result);
            }
    ...
    $("#file_interrieur_Ajoutvendeur_img").change(function(){
        readURL(this);
    });
