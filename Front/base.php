...
<?php
    ...
    $queryItem = mysqli_query($con, "SELECT price,name,category,subcategory,quantity,description,photo,end_enchere FROM item WHERE id_item='3'");
?>
...
if($rowItem = mysqli_fetch_assoc($queryItem)){
                ...
                $queryBuyer = mysqli_query($con, "SELECT id_buyer FROM buyer WHERE email='laulau@yahoo.com'");

                if($row = mysqli_fetch_assoc($queryBuyer)){
                    $id_buyer = $row['id_buyer'];
                
                $query = mysqli_query($con, "select count(DISTINCT id_buyer) as total from bid where id_item='3'");
