<?php
    ...
    require "head.php";
        ...
        $queryCountItems = mysqli_query($con, "select count(id_item) as total from item where category='Clothes' and subcategory='shoes'"); //4
        $queryAllItems = mysqli_query($con, "select id_item,name, price , brand,subcategory, quantity, description,photo, is_negotiated, is_buying from item,seller where seller.id_seller=item.id_seller and category='Clothes'and subcategory='shoes' and (is_buying =1 or is_negotiated=1)");// 
?>
...
<div class="vetement_general">
    ...
    <div class="deco_marque_general position-absolute position-absolute top-0 end-0">
        ...
        <p class="texte_deco_marque_general texte_style text-uppercase position-absolute top-50 start-50 translate-middle">Clothes</p> 
    </div>
